...
export async function POST(req) {
  try {
    ...
    if (existingDevice) {
      const user = existingDevice.user;

      if (user?.isAnonymous) {
        // Generate tokens for existing anonymous user
        const accessToken = await generateAccessToken(user.id, existingDevice.id);
        const refreshToken = await generateRefreshToken(user.id, existingDevice.id);

        return NextResponse.json(
          {
            success: true,
            message: "Anonymous user exists.",
            data: {
              userId: user.id,
      ...
      // User is not anonymous → require login
      return NextResponse.json(
        {
          success: false,
